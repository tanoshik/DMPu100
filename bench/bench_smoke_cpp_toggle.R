# bench/bench_smoke_cpp_toggle.R
# Smoke benchmark for run_match_fast() with use_cpp=FALSE vs TRUE.
# Target: small DB (1K) to verify both paths run and measure elapsed + throughput.
...
read_db_indexed <- function(rds_path) {
  ...
  if (!all(exp %in% names(db))) {
    stop("DB RDS must be an indexed list with: sample_ids, locus_ids, A1, A2")
  }
  # light checks
...
bench_once <- function(db_rds,
  ...
  # estimate evaluations = (#non-any-any loci) * (#samples)
...
write_kpi <- function(kpi_row, out_csv) {
  ...
  line <- paste(kpi_row, collapse = ",")
  if (hdr) {
    header <- paste(names(kpi_row), collapse = ",")
...
# ---- Main (params) ----
# Default DB: 1K virtual (fast smoke)
db_rds <- getOption("dmp.smoke.db_rds", "data/virtual_db_u100_S1000_seed123.rds")
include_bits <- FALSE  # for I/O reduction in bench
out_csv <- "test/bench/bench_kpi_100u_all.csv"  # append to the unified KPI

.ensure_dir(dirname(out_csv))

cat(sprintf("[SMOKE] DB=%s  include_bits=%s\n", db_rds, include_bits))

# R path
cat("[SMOKE] use_cpp=FALSE ...\n")
...
cat("\n[SMOKE] Done. KPI appended to: ", out_csv, "\n", sep = "")
